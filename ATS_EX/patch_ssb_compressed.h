#pragma once 
// Credit for the clever patch compression method goes to den3rats

// cutoff line positions as 1-byte offsets from segment base
const PROGMEM uint8_t cutoff_places_offsets[] = {
    /* base 0   */ 127,
    /* base 256 */ 114, 117, 119, 121, 123, 126, 128, 131, 133, 135, 137, 139, 141, 143, 145, 147,
    /* base 512 */ 80, 103, 137, 148, 244,
    /* base 768 */ 100, 103, 146, 173, 176, 179, 216, 225, 230, 233, 235, 238, 241, 244, 246, 250, 253,
    /* base 1024*/ 15, 38, 44, 49, 51, 53, 56, 59, 63, 66, 68, 72, 74, 77, 79 
};

// cutoff payload lengths
// n in [1..99]  -> n data bytes then zeros,      next line starts with 0x15 00 00
// n in [100..] -> (n-100) data bytes then zeros, next line starts with 0x15 00
const PROGMEM uint8_t cutoff_nonzero_lengths[] = {
    /* base 0   */ 103,
    /* base 256 */ 2, 2, 5, 5, 5, 2, 5, 2, 2, 2, 3, 2, 2, 3, 2, 102,
    /* base 512 */ 8, 6, 5, 106, 103,
    /* base 768 */ 7, 106, 6, 6, 6, 6, 4, 8, 4, 2, 5, 2, 2, 2, 5, 7, 8,
    /* base 1024*/ 7, 8, 3, 6, 7, 7, 8, 2, 3, 4, 3, 7, 3, 4, 3
};

// Reduced ssb patch - without cutoff places!
const PROGMEM byte compressed_ssb_patch_content[] =
{
    , //0x00, 0x00, 0x00, 0x00, 0x00, //127 2 bytes
/*0x00,*/ 0x06, 0x98, 0x69, 0x15, 0x1C, 0xFC,   //128 short 0x15 0x00 continue
    , //0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //370 1 byte
/*0x00, 0x00,*/ 0x08, 0xE3, 0x51, 0x84, 0x00,   //371 0x15 0x00 0x00 continue
    0x0B, 0x4D, 0x26, 0x73, 0x11, 0x8B, 0x1E,
    0xE2, //0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //373 1 byte
/*0x00, 0x00,*/ 0x04, 0x04, 0x73, 0xE0, 0x72,   //374 0x15 0x00 0x00 continue
    0xFF, 0xE6, 0x59, 0xC9, //0x00, 0x00, 0x00, //375 4 bytes
/*0x00, 0x00,*/ 0x04, 0x48, 0xBF, 0xAB, 0xF3,   //376 0x15 0x00 0x00 continue
    0x99, 0x87, 0x7C, 0x07, //0x00, 0x00, 0x00, //377 4 bytes
/*0x00, 0x00,*/ 0x04, 0xFB, 0x33, 0xB1, 0x63,   //378 0x15 0x00 0x00 continue
    0x61, 0x61, 0x12, 0xAD, //0x00, 0x00, 0x00, //379 4 bytes
/*0x00, 0x00,*/ 0x08, 0xAA, 0x30, 0xAC, 0x17,   //380 0x15 0x00 0x00 continue
    0x4D, 0x30, 0x86, 0xBF, 0x67, 0x13, 0xF1,
    0xA7, //0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //382 1 byte
/*0x00, 0x00,*/ 0x04, 0x1B, 0x48, 0x42, 0x23,   //383 0x15 0x00 0x00 continue
    0x84, 0xBB, 0x7F, 0xB4, //0x00, 0x00, 0x00, //384 4 bytes
/*0x00, 0x00,*/ 0x08, 0xA6, 0x9F, 0x43, 0x78,   //385 0x15 0x00 0x00 continue
    0x5C, 0xA1, 0x4E, 0x8B, 0xA6, 0xE6, 0x7C,
    0x49, //0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //387 1 byte
/*0x00, 0x00,*/ 0x01, 0xF7, 0x31, 0x5D, 0x27,   //388 0x15 0x00 0x00 continue
    0x30, //0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //389 1 byte
/*0x00, 0x00,*/ 0x01, 0x0B, 0x57, 0x0B, 0x79,   //390 0x15 0x00 0x00 continue
    0xF7, //0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //391 1 byte
/*0x00, 0x00,*/ 0x02, 0x44, 0x33, 0xBB, 0xB4,   //392 0x15 0x00 0x00 continue
    0x5B, 0xC7, //0x00, 0x00, 0x00, 0x00, 0x00, //393 2 bytes
/*0x00, 0x00,*/ 0x01, 0xAB, 0xE3, 0x01, 0xC2,   //394 0x15 0x00 0x00 continue
    0xD2, //0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //395 1 byte
/*0x00, 0x00,*/ 0x01, 0x7A, 0x07, 0x9E, 0xAD,   //396 0x15 0x00 0x00 continue
    0x84, //0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //397 1 byte
/*0x00, 0x00,*/ 0x02, 0xF1, 0xEE, 0xF7, 0xD8,   //398 0x15 0x00 0x00 continue
    0x29, 0x50, //0x00, 0x00, 0x00, 0x00, 0x00, //399 2 bytes
/*0x00, 0x00,*/ 0x01, 0x2C, 0x35, 0x54, 0x3A,   //400 0x15 0x00 0x00 continue
    0xCC, //0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //401 1 byte
/*0x00, 0x00,*/ 0x01, 0xC9, 0x3D, 0x66, 0x6A,   //402 0x15 0x00 0x00 continue
    0x98, //0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //403 1 byte
/*0x00,*/ 0x05, 0x24, 0x7A, 0xF7, 0x1C, 0x24,   //404 short 0x15 0x00 continue
    ,   //592 7 bytes
/*0x00, 0x00,*/ 0x98, 0xF7, 0xB0, 0xF9, 0xFA,   //593 0x15 0x00 0x00 continue
    , //0x00, 0x00,   //615 5 bytes
/*0x00, 0x00,*/ 0xE4, 0xF2, 0xEA, 0x7A, 0x73,     //616 0x15 0x00 0x00 continue
    , //0x00, 0x00, 0x00, //649 4 bytes
/*0x00, 0x00,*/ 0x44, 0x89, 0x2C, 0x63, 0xC8,   //650 0x15 0x00 0x00 continue
    0xAF, 0x7B, 0xAC, 0x35, 0x43, 0x27, 0xBE,
    0x7E, 0xCF, 0x56, 0x40, 0x3D, 0xA5, 0xAC,
    0x0F, 0x97, 0xCC, 0xD4, 0x6E, 0xD0, 0x31,
    0xD7, 0x14, 0x60, 0x45, 0xAE, 0xDF, 0x0A,
    0xAA, 0x33, 0xB3, 0x88, 0xE0, 0x1C, 0x26,
    0xDC, 0x00, 0xB8, 0x75, 0xC8, 0x67, 0x11,
    0xA1, 0xB5, 0x87, 0x71, 0xF9, 0x4A, 0x6A,
    0xEC, 0x3F, 0x0E, 0x41, 0x21, 0xC2, 0x4F,
    0x92, 0x8F, 0x17, 0x53, 0xEE, 0xD5, 0xEA,
    0x4F, 0x91, 0x75, 0x02, 0x59, //0x00, 0x00, //660 5 bytes
/*0x00,*/ 0x02, 0x94, 0xE4, 0xDA, 0x4A, 0xB3,   //661 short 0x15 0x00 continue
    , //0x00, 0x00, 0x00, 0x00, 0x00, //756 2 bytes
/*0x00,*/ 0x03, 0x08, 0x75, 0x7D, 0x9A, 0x49,   //757 short 0x15 0x00 continue
    , //0x00, //868 6 bytes
/*0x00, 0x00,*/ 0x0C, 0x80, 0x1F, 0xB4, 0xF4,   //869 0x15 0x00 0x00 continue
    0xE0, 0xA6, 0x41, 0xCC, 0x87, 0x00, 0x40,
    0x61, 0x43, 0xE7, 0xFC, 0xB4, //0x00, 0x00, //871 5 bytes
/*0x00,*/ 0x01, 0x24, 0xCA, 0x45, 0x34, 0x25,   //872 short 0x15 0x00 continue
    , //0x00, 0x00, //914 5 bytes
/*0x00, 0x00,*/ 0xB4, 0x89, 0x91, 0x34, 0x9B,   //915 0x15 0x00 0x00 continue
    , //0x00, 0x00, //941 5 bytes
/*0x00, 0x00,*/ 0x0C, 0x6F, 0xE6, 0x5C, 0x04,   //942 0x15 0x00 0x00 continue
    0x30, 0x6C, 0x84, 0xC7, 0x51, 0xE3, 0xB5,
    0x5F, 0x34, 0x9E, 0xDD, 0x05, //0x00, 0x00, //944 5 bytes
/*0x00, 0x00,*/ 0x0C, 0x1C, 0xD4, 0xBC, 0xFB,   //945 0x15 0x00 0x00 continue
    0xCD, 0x3A, 0xAE, 0xBC, 0x52, 0x92, 0x54,
    0x85, 0x2C, 0xED, 0x0F, 0xFB, //0x00, 0x00, //947 5 bytes
/*0x00, 0x00,*/ 0xF8, 0x73, 0x10, 0x5D, 0xD8,   //948 0x15 0x00 0x00 continue
    , //0x00, 0x00, 0x00, 0x00, //984 3 bytes
/*0x00, 0x00,*/ 0x38, 0x97, 0x4C, 0x36, 0xA2,   //985 0x15 0x00 0x00 continue
    0x75, 0xFF, 0xFB, 0x3F, 0xEB, 0x60, 0xE1,
    0x31, 0xB1, 0xBC, 0x32, 0x77, 0x6C, 0xBD,
    0x49, 0x93, 0x4B, 0xC2, 0x07, 0xBA, 0xF0,
    0x67, 0xFD, 0x68, 0xCA, 0xF5, 0xA4, 0xFF,
    0x2A, 0x5E, 0xE2, 0xBE, 0xB5, 0x69, 0xB2,
    0xA0, 0x63, 0x93, 0x1B, 0xB4, 0xBD, 0x46,
    0x12, 0xFA, 0xEE, 0x88, 0x5F, 0x46, 0xDF,
    0xB2, 0xBA, 0xE8, 0xBA, 0x54, 0x47, 0xEA,   //993 7 bytes
/*0x00, 0x00,*/ 0x18, 0xFA, 0x08, 0x91, 0x77,   //994 0x15 0x00 0x00 continue
    0x9B, 0x1C, 0x87, 0x8B, 0x01, 0x70, 0x9E,
    0xC0, 0x05, 0x2B, 0xAA, 0xD7, 0xAF, 0x30,
    0xA8, 0x01, 0x8B, 0x28, 0xA0, 0xF5, 0x95,
    0x99, 0x32, 0x5F, //0x00, 0x00, 0x00, 0x00, //998 3 bytes
/*0x00, 0x00,*/ 0x08, 0xC4, 0xEF, 0x65, 0x45,   //999 0x15 0x00 0x00 continue
    0x3C, 0xC3, 0x6D, 0xEF, 0xB8, 0x34, 0x9D,
    0x1C, //0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //1001 1 byte
/*0x00, 0x00,*/ 0x04, 0x66, 0x86, 0xF2, 0xBC,   //1002 0x15 0x00 0x00 continue
    0x9D, 0x87, 0x97, 0xBE, //0x00, 0x00, 0x00, //1003 4 bytes
/*0x00, 0x00,*/ 0x08, 0xDD, 0x58, 0xDD, 0x53,   //1004 0x15 0x00 0x00 continue
    0x0F, 0x7D, 0xEC, 0x93, 0x0B, 0xB9, 0x5B,
    0xE3, //0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //1006 1 byte
/*0x00, 0x00,*/ 0x08, 0x76, 0x15, 0xDF, 0xC7,   //1007 0x15 0x00 0x00 continue
    0x68, 0x2F, 0xA8, 0xF8, 0x4D, 0x33, 0x8F,
    0xAE, //0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //1009 1 byte
/*0x00, 0x00,*/ 0x08, 0x84, 0xC3, 0xE8, 0xCB,   //1010 0x15 0x00 0x00 continue
    0x9C, 0x71, 0x80, 0x09, 0xD7, 0x11, 0x63,
    0x21, //0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //1012 1 byte
/*0x00, 0x00,*/ 0x04, 0x0E, 0xC6, 0xF0, 0x50,   //1013 0x15 0x00 0x00 continue
    0x9E, 0x0A, 0xA7, 0x99, //0x00, 0x00, 0x00, //1014 4 bytes
/*0x00, 0x00,*/ 0x14, 0x9A, 0x5A, 0xF9, 0xAA,   //1015 0x15 0x00 0x00 continue
    0x5F, 0x06, 0xDF, 0x5C, 0x52, 0x37, 0x4B,
    0x39, 0x0C, 0x7E, 0x18, 0xC1, 0xAB, 0xD6,
    0x36, 0xF5, 0x78, 0xBD, 0x9F, 0x79, //0x00, //1018 6 bytes
/*0x00, 0x00,*/ 0x0E, 0xF2, 0x33, 0x61, 0x0E,   //1019 0x15 0x00 0x00 continue
    0xC3, 0x0B, 0x5A, 0x24, 0x02, 0x3F, 0x42,
    0x35, 0x3D, 0xA4, 0xAD, 0xC5, 0x30, 0x50,   //1021 7 bytes
/*0x00, 0x00,*/ 0x76, 0xE7, 0xE3, 0x45, 0x08,   //1022 0x15 0x00 0x00 continue
    0x66, 0x18, 0x21, 0xEB, 0xDD, 0x14, 0x76,
    0xF8, 0xF1, 0x55, 0xE8, 0x3B, 0x9D, 0x8B,
    0x93, 0x92, 0xFF, 0xA7, 0x80, 0x34, 0x38,
    0xC2, 0xAD, 0x7C, 0xFF, 0xBD, 0x23, 0x2B,
    0x0B, 0x2C, 0x5F, 0xEA, 0xC0, 0x95, 0x00,
    0xFB, 0x8C, 0x37, 0x6B, 0x8A, 0x42, 0x91,
    0x3E, 0xEF, 0xD6, 0x51, 0x43, 0x21, 0x6F,
    0x80, 0x3E, 0xBD, 0xF2, 0xB3, 0x24, 0x07,
    0xA5, 0x92, 0x4A, 0x76, 0x51, 0x31, 0x72,
    0x35, 0x47, 0x25, 0xE7, 0xB9, 0xD0, 0xF4,
    0xC2, 0xDD, 0x52, 0x35, 0x3A, 0x23, 0xC4,
    0x16, 0xFA, 0x2A, 0x8D, 0x99, 0xD0, 0xB9,
    0x53, 0xC2, 0x12, 0x5C, 0x3F, 0xCE, 0x8D,
    0xDB, 0xD1, 0xC8, 0xD1, 0x59, 0xE5, 0x88,
    0xFF, 0x49, 0x11, 0xAB, 0xAB, 0x7B, 0xD1,
    0xF2, 0xB4, 0x37, 0x49, 0xA0, 0xB1, 0xFE,
    0xFE, 0x6C, 0x08, 0x76, 0x1D, 0x0B, //0x00, //1039 6 bytes
/*0x00, 0x00,*/ 0x9A, 0xF4, 0xB1, 0x6D, 0xA0,   //1040 0x15 0x00 0x00 continue
    ,   //1062 7 bytes
/*0x00, 0x00,*/ 0x1E, 0xB0, 0x02, 0x8E, 0x45,   //1063 0x15 0x00 0x00 continue
    0xBD, 0xF0, 0xA5, 0x90, 0xB7, 0x30, 0x12,
    0x40, 0xB6, 0x40, 0x9C, 0xF1, 0x62, 0x63,
    0x72, 0x6D, 0xF0, 0xBA, 0x1A, 0xF6, 0xF0,
    0x40, 0x6A, 0x47, 0xA2, 0xFC, 0x9B, 0xD8,
    0x70, 0x19, //0x00, 0x00, 0x00, 0x00, 0x00, //1068 2 bytes
/*0x00, 0x00,*/ 0x1A, 0xF5, 0x24, 0xF6, 0x4D,   //1069 0x15 0x00 0x00 continue
    0x2F, 0x64, 0xFD, 0xF7, 0xC3, 0xFD, 0xA7,
    0x45, 0x5D, 0x94, 0x31, 0x8F, 0xD6, 0x9F,
    0x5D, 0x21, 0x66, 0x41, 0xD4, 0x39, 0x51,
    0xAF, 0x61, 0x8C, 0x1E, 0x9F, //0x00, 0x00, //1073 5 bytes
/*0x00, 0x00,*/ 0x06, 0x11, 0xE9, 0x7A, 0x88,   //1074 0x15 0x00 0x00 continue
    0x31, 0xE1, 0x26, 0xB6, 0x1C, 0x74, //0x00, //1075 6 bytes
/*0x00, 0x00,*/ 0x06, 0xF5, 0x98, 0x35, 0x17,   //1076 0x15 0x00 0x00 continue
    0x1A, 0xAB, 0x19, 0x3A, 0x8C, 0xC6, //0x00, //1077 6 bytes
/*0x00, 0x00,*/ 0x0E, 0x7E, 0x14, 0xD0, 0x6E,   //1078 0x15 0x00 0x00 continue
    0x44, 0x1C, 0x66, 0x61, 0x67, 0xF1, 0x8E,
    0x28, 0x72, 0x30, 0xD2, 0xBF, 0x7D, 0x6F,   //1080 7 bytes
/*0x00, 0x00,*/ 0x08, 0x17, 0x99, 0x62, 0xEE,   //1081 0x15 0x00 0x00 continue
    0x78, 0x18, 0x54, 0x3A, 0xD5, 0x7F, 0x61,
    0xAC, //0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //1083 1 byte
/*0x00, 0x00,*/ 0x10, 0x83, 0x3E, 0x2C, 0x92,   //1084 0x15 0x00 0x00 continue
    0x4F, 0x5A, 0x98, 0x59, 0x97, 0xFB, 0x82,
    0x97, 0xAD, 0xC8, 0x73, 0xA2, 0x6C, 0x0D,
    0x06, 0xCF, //0x00, 0x00, 0x00, 0x00, 0x00, //1087 2 bytes
/*0x00, 0x00,*/ 0x0A, 0x69, 0xDA, 0xEB, 0x9E,   //1088 0x15 0x00 0x00 continue
    0xF7, 0x67, 0x7F, 0xDE, 0xE8, 0x83, 0x0B,
    0x46, 0x5A, 0x3F, //0x00, 0x00, 0x00, 0x00, //1090 3 bytes
/*0x00, 0x00,*/ 0x02, 0xB3, 0x60, 0x7F, 0xDC,   //1091 0x15 0x00 0x00 continue
    0xA3, 0xFF, //0x00, 0x00, 0x00, 0x00, 0x00, //1092 2 bytes
/*0x00, 0x00,*/ 0x14, 0x8E, 0xCF, 0xA2, 0x65,   //1093 0x15 0x00 0x00 continue
    0x72, 0xCE, 0x3E, 0x11, 0x39, 0x2D, 0x94,
    0x94, 0x47, 0x8E, 0xC7, 0x8C, 0x70, 0x6B,
    0x7C, 0x49, 0x8E, 0x3C, 0xD7, 0x4C, //0x00, //1096 6 bytes
/*0x00, 0x00,*/ 0x02, 0xC8, 0x77, 0xA5, 0xFD,   //1097 0x15 0x00 0x00 continue
    0xBA, 0x9A, //0x00, 0x00, 0x00, 0x00, 0x00, //1098 2 bytes
/*0x00, 0x00,*/ 0x0A, 0x1A, 0x1B, 0xD9, 0x4A,   //1099 0x15 0x00 0x00 continue
    0x83, 0x03, 0xDF, 0xE2, 0x75, 0x6E, 0x28,
    0xD9, 0x63, 0x1F, //0x00, 0x00, 0x00, 0x00, //1101 3 bytes
/*0x00, 0x00,*/ 0x02, 0x18, 0x27, 0x27, 0x8F,   //1102 0x15 0x00 0x00 continue
    0x4B, 0xE7, //0x00, 0x00, 0x00, 0x00, 0x00, //1103 2 bytes
/*0x00, 0x00,*/ 0x00, 0x00, 0x00, 0xA9, 0x02    //1104 0x15 0x00 0x00 continue
};


// ===============================================================
// ======== OLD LOGIC SSB PATCH WITH ZERO BYTES =================
// ===============================================================

const PROGMEM uint16_t cmd_0x15[] =
{
       0,   128,  371,  374,  376,  378,  380,  383,  385,  388,  390,  392,  394, 396, 398, 400,
     402,   404,  593,  616,  650,  661,  757,  869,  872,  915,  942,  945,  948, 985, 994,
     999,  1002, 1004, 1007, 1010, 1013, 1015, 1019, 1022, 1040, 1063, 1069, 1074,
     1076, 1078, 1081, 1084, 1088, 1091, 1093, 1097, 1099, 1102, 1104
};

const PROGMEM byte ssb_patch_content[] =
{
    
};
